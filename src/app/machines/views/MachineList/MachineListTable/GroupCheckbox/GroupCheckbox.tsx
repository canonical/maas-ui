...
import type { FilterGroupOptionType } from "app/store/machine/types";
import type { RootState } from "app/store/root/types";

type Props = {
  callId?: string | null;
  groupName: string;
  groupKey: FilterGroupOptionType;
};

const GroupCheckbox = ({ callId, groupName, groupKey }: Props): JSX.Element => {
  const selected = useSelector(machineSelectors.selectedMachines);
  const group = useSelector((state: RootState) =>
    machineSelectors.listGroup(state, callId, groupKey)
  );
  const allSelected = !!selected && "filter" in selected;
  // Whether this group is currently selected.
  const groupSelected =
    !!selected && "groups" in selected && selected.groups?.includes(groupKey);
  ...
  return (
    <TableCheckbox
      ...
      onGenerateSelected={(checked) => {
        ...
        if (checked && !newSelected.groups?.includes(groupKey)) {
          // If the checkbox has been checked and the group is not in the list
          // then add it.
          newSelected.groups.push(groupKey);
        } else if (!checked && newSelected.groups?.includes(groupKey)) {
